TODOs:
  check for @Asynchronous on participant termination methods (or check that such methods return the Accept header
  replace the URLs for looking up LRAs and for participant status with HTTP query parameters
  @Status must be combined with @Forget
    ...
    - [Proposed solution](#proposed-solution)
      ...
      - [CDI Annotations for LRAs](#cdi-annotations-for-lras)
        - [Java Annotations](#java-annotations)
          ...
          - [Timing out LRAs and Compensators](#timing-out-lras-and-participants)
...
* Fail: the activity has completed unsuccessfully. All participants that
  ...
  is in progress but not complete. Furthermore a participants can indicate
...
<a name="participant-state-model"></a>
   ...
   - `Compensating`: a participant is currently compensating for the LRA;
   ...
   - `Completed`: the participant has confirmed.
...
This should make the specification useable by a wider variety of developers:
...
The lifetime of the LRA can be specified using the @TimeLimit annotaton
which has attributes for defining the unit and period.
...
Similarly, if the developer has provided an @Complete method it will be invoked
if the LRA is closed.

If the bean cannot perform a compensation or completion activity immediately the
termination method should be marked as a JAX-RS asynchronous method (using the
javax.ws.rs.container.Suspended annotation). In this case the LRA coordinator will need
to monitor the progress of the participant and the developer must
provide a @GET method annotated with @Status which must returns a string representation
...
task synchronously. The response entity must correspond to one of the following
enum values:
...
If the participant is in none of these states it should report the error
using a JAX-RS exception mapper that maps to a `400 Bad Request` HTTP status
code.
...
where the possible values for the status of a participant is captured in
...
The participant is registered using an instance of the following interface:
...
How the application obtains an LRAManagement instance is unspecified (for example the reference implementation
does it by CDI injection). The deserializer matches the interface
...
where base-uri is `\<protocol\>://\<hostname\>` (such as http://localhost:8080).
   ...
   * GET {base uri}/lra-coordinator/?status={Completing|Compensated|etc}
     returns a subset of all LRAs depending upon the status query parameter.
...
associated with the new LRA and can be retieved when the LRA is
status is reported.
...
1. Performing a `GET` on `\<base uri\>/lra-coordinator/\<LRAId\>`
   returns the status of the LRA. If the request contains an accept
   header of type `application/json` the request returns `200 OK`
   if the LRA is know to exist. The content will contain a
   [JSON representation of its current status](#status-format).
   If there is no such accept header and the the LRA is known to exist
   the response is `204 No Content`.

   Clients that do not accept json can still discover the status of
   the LRA using query parameters:

   - `status=\<Completed\>`
   - `status=\<Compensated\>`
   - `status=\<Completed\>`

   - performing a `GET` on `\<base uri\>/lra-coordinator/\<LRAId\>/completed`
     returns `204` if the LRA completed successfully.

   - performing a `GET` on `\<base uri\>/lra-coordinator/\<LRAId\>/compensated`
     returns `204` if the LRA compensated.
...
4. Performing a `PUT` on `\<base uri\>/lra-coordinator/\<LRAId\>/close`
   ...
   each participant). Upon termination, the URL is implicitly deleted. If
   ...
   participant. The response body of the request may contain a JSON array of
   application specific strings (one for each participant).
...
5. Performing a `PUT` on `\<base uri\>/lra-coordinator/\<LRAId\>/cancel`
   ...
   to each participant). The response body of the request may contain a JSON
   array of application specific strings (one for each participant).
...
If any participant cannot complete/compensate immediately (either because
it is in progress or because of a failure) then the LRA status is
reported as recovering. Recovering LRAs will be periodically retried.
...
for completing or compensating for the action, for reporting its` status
and for being notified that it can clean up and forget about this LRA.

We call these endpoints the `compensatation URL`, `completion URL`,
`status URL` and `forget URL` and they have the following semantics:
...
Participant enlistement in a LRA is described below.

###### Participant Completion



If there is a response body to this registration request it will be
retained by the coordinator and returned to the participant when the
termination phase is entered.

if present, is remembered and returned back to the participant when the
LRA is closed or cancelled (ie when the coordinator sends a PUT request
to the `completion URL` or `compensation URL` endpoint). This feature can
be used by participants for reliably storing data which may be helful
to the participant for deciding how to complete or compensate for an
action. In particular, the data survives JVM failures. 
Any 
###########

Providing status and forget URLs at registration time is optional, but
if absent, then they must be provided by the participant during the 
termination phase if it cannot successfuly complete the request immediately.
In this case it must provide the status URL in the Location header of the
response. An HTTP `GET` request on this URL should report the status of the
compensator (see below). If the participant has not yet entered the
termination phase it should respond to status requests with
`412 Precondition Failed` and in this case can expect to receive a
subsequent termination request.

 and HTTP `DELETE` tells the participant that it is safe to clean
up and forget about this LRA. If the compensator has not yet been asked to
enter the end phase of the protocol it should respond to status requests
with `412 Precondition Failed`.

If the response does not reach the coordinator
and the coordinator does not have the status URL its` only recourse is to
retry the completion phase. Therefore complete and compensate should be
idempotent operations (as is normal for `PUT` requests). The participant
can avoid the idempotency requirement by providing the status and forget
URLs at registration time.

A participant can register with an LRA in one of two alternative ways:

1. Using a PUT request to the LRA URL together with xxxx whose body contains a single
   `\<participant URL\>`. The convention is that the
...
```
   ...
   used similarly to alternative registration mechanism 1 and the
   remaining relations will be ignored. Also as mentioned previously
   the status and forget relations are optional.
   
   Again the URI-Reference part of each link component (the one between
   the angle brackets) must be unique within the context of a given LRA.
   Also notice that the participant is free to use different URLs
...
It is expected that specification implementers durably record which
...
The participant should respond to termination requests as follows:
   ...
   1. `200 OK` with an optional body;
   2. `204 No Content` and no body;
   3. `202 Accepted` if it cannot terminate immediately;

If the response has a body then the coordinator will make it available
to whoever closed/cancelled the LRA in the form of a JSON array (one for
each participant that enlisted in the LRA).

In the case of a 202 response the `Location` header, if present, will be
used in place of any existing participant status and forget URLs.

If the coordinator receives any response other than 200 or 204 it
will periodically retry.

When asked to complete or compensate a participant should return
`200 OK` or `204 No Content` to indicate success unless it is unable to
satisfy the request immediately in which case it should return
`202 Accepted` and report its status later by providing a status URL in
the HTTP `Location` header field of the response. If this header is absent
the status url used at registration time will be used. If no
such status URL is provided then the participant will be marked as
permanently failed (ie returning 202 and not providing a status URL is
unsafe).  If the participant is unknown then `410 Gone` or `404 Not Found`
will be returned and it can be assumed by the coordinator that the service
compensated.

If the participant responds with `200 OK` the content in the response
body will be made available to whoever closed/cancelled the LRA in the
form of a JSON array (one for each participant that enlisted in the LRA).

Note that if a participant cannot compensate it must maintain this
information until it is told to forget via a `DELETE` request to status
URL.

As remarked above the service writer must take into account that
complete and compensate endpoints may be called multiple times, so the
operations must be idempotent.

###### Reporting the Participant Status

A participant reports its status via the status URL. Performing a `GET`
on this URL will return the current status of the participant in the
...
compete or compensate. Valid status values are:

   - `Compensating`: the participant is currently compensating for the LRA;
   - `Compensated`: the participant has successfully compensated for the LRA.
   - `FailedToCompensate`: the participant was not able to compensate for
      the LRA. It must maintain information about the work it was to
      compensate until the coordinator sends it a forget message.
   - `Completing`: the participant is tidying up after being told to complete.
...
request to the forget URL. On success the coordinator remove knowledge
of the participant (ie will no longer send requests on the participant).
If the `DELETE` request fails, the recovery system will periodically
replay delete request. (TODO make sure the TCK has a test for this.)
...
no longer send requests on the participant).
...
status. A participant is placed in this state if:
   ...
   - the compensate or complete response was anything other than
     `200 OK` (TODO should any codes move it immediately to
     FailedToCompensate or FailedToComplete?);
...
uri\>/lra-coordinator/\<LRAId\>/remove` with the URL of the participant in
