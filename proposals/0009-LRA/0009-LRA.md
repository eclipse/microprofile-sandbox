...
coordinate short running activites in such a way as to guarantee
a globally consistent outcome but this time with the possibility that 
it may be neccessary to lock data.
...
- support short running actions that may need to be started to
  facilitate the exection of a step in a long running action
...
This set of APIs means that the specification will useable in a wider variety of systems:
...
public @interface LRA {
     ...
     * compensatable transaction (aka LRA) context where the values provide the following behaviors:
     */
    enum Type {
        /**
...
The bean indicates that it cannot finish immediately by by either
   ...
   - or the mehtod is marked as a JAX-RS asynchronous method (using the
...
[value of the status query parameter](#participant-state-model)
   ...
   * To find all active LRAs (ie ones that have not yet been asked to close
     or cancel specifiy and empty status:
...
The status URLs is only required if the participant cannot
...
There are two alternatvive forms of registration:
...
```
   As mention earlier, the status URL will only be used if coordinator
   ...
   remaining relations will be ignored.
...
The participant should respond to termination requests as follows:
   ...
   1. `204 No Content` and no body
      ...
      `204 Not Found` in which case the coordinator assumes that
      the participant must have previously terminated successfully.
   2. `202 Accepted` if it cannot terminate immediately
      to indicate that the participant cannot finish immediately
      and the coordinator should monitor the eventual outcome via
      the `status URL`
      The particpant MAY include a `status` URL in the `Location`
      ...
      a status URL it will periodcially retry the completion request.
   ...
   3. `200 OK` with a content body
      ...
      The coordinator will log the particant as failed and no longer
      make requests to it.

If the coordinator receives any response other response codes it will
...
of the participant (ie will no longer send requests to the participant).
