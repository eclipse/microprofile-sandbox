...
participants) make changes visible but register a compensatory action
...
This client API still requires that participants listen on REST endpoints.
...
API for participants that avoids having to explicitly support HTTP. See
...
`@GET` and `@Status` which should report the status of the participants as
...
annotated with a `@PUT` and `@Forget` annotations.
...
If a participant is unable to compensate then it must remember the fact
...
LRAs and participant. To support this class of user an instance of [client
API](lra-annotations/src/main/java/org/eclipse/microprofile/lra/client/LRAClientAPI.java)
can be injected.
...
``` java
/**
 * The API for notifying participant that an LRA is completing or cancelling
 */
...
public interface LRAManagement {
    /**
     * join an LRA * @param participant an instance of a {@link Compensator} that will be notified when the target LRA ends 
     * @param deserializer a mechanism for recreating participant during recovery 
     * @param lraId the LRA to join 
     */
    void joinLRA(Compensator participant, CompensatorDeserializer deserializer,
    String lraId);
} 
```
...
Once a resource has the LRA context, it can register participation in the
LRA (ie enlist as a participant). It does so by providing endpoints for
completing or compensating for the action, for reporting its` status and
for being notified that it can clean up and forget about this LRA. The
status and forget URLs are only required if the participant is unable to
complete or compensate immediately when asked to do so.
...
`status URL` and `forget URL` and they have the following semantics:
   ...
   - PUT `completion URL` tells the participant to complete the action;
   - PUT `compensatation URL` tells the participant to compensate for
     the action;
...
The status and forget URLs are optional. If the forget URL is
absent then the status URL (if present) will be used in place of forget.
As is usual with PUT, complete and compensate should be idempotent
operations.

A participant can register with an LRA in one of two alternative ways:

1. Using a PUT request to the LRA URL whose body contains a single
